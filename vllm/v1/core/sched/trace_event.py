...
def snapshot_kv_connector_state(state: Any) -> dict[str, Any] | None:
    """Copy CPU block hand-offs before connector consumption and async writing."""
    if state is None:
        return None
    return {
        "block_ids": {
            req_id: [list(group) for group in groups]
            for req_id, groups in state.block_ids.items()
        },
        "boundary_state_offloads": {
            req_id: [list(entry) for entry in entries]
    ...
    *,
    ...
    return {
        ...
        "kv_connector": {
            ...
            "offered_block_state": kv_connector_state,
        },
        "token_budget": {
